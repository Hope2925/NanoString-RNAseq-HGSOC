...
def get_final_nr_dataset(nr_data_set_dict, NanovRNAseq_pat_ho_dict, nano_genes):
	...
	* the df with all the values for possible future usage (nr_all_df) (no hold out)
	...
	nr_new_data_set_dict = {"Manso_Adzib":man_adzib_df, "Manso_Jav":man_jav_df,  "Manso_EGA":man_ega_df, 
					...
					"All":nr_all_df, "Hold_Out":pd.concat(X_final_val_nr, axis=0)}
...
def get_final_pfs_dataset(data_set_dict, PFS_pat_ho_dict, nano_genes, prediction_mth=12, RNAseqonly=False):
	...
	* the df with all the values for possible future usage (all_df) (no hold out)
	...
	if PFS_pat_ho_dict == "Adzib":
		...
		new_data_set_dict["All"] = pd.concat([new_data_set_dict["RNAseq_J"], new_data_set_dict["RNAseq_EGA"]], axis=0)
		...
		if RNAseqonly is False:
			...
			new_data_set_dict["All"] = pd.concat([new_data_set_dict["Nano"], new_data_set_dict["RNAseq"]], axis=0)
			...
			new_data_set_dict["All"] = pd.concat([new_data_set_dict["RNAseq_A"], new_data_set_dict["RNAseq_J"], new_data_set_dict["RNAseq_EGA"]], axis=0)
...
def get_gene_Fscores(data_set_dict_use):
	...
	for dataset, (X_use, y_use, y_clean_use, meta_use) in data_set_dict_use.items():   
		if dataset in ["Hold_Out", "All", "All_Filt", "RNAseq", "Nano", "All_Assay"]:
			continue
...
#####################################################
def train_and_eval_loocv(model, X, y, k, features_list=None, exhaustive=False):
	"""
    ...
    * results: pandas dataframe with K, gene subset, and auc
	...
	for gene_subset in gene_sets:
		...
		for train_idx, test_idx in loo.split(X):
			...
			# feature selection
			X_train_sel = X_train[gene_subset]
			X_test_sel = X_test[gene_subset]
			# train and predict
...
def train_eval_main(X, y, data_use="Nano", plot_data_set_dict_use=None, gene_set_sizes=[5, 10, 20, 25, 30, 35, 40, 45, 50], feature_list=None, exhaustive=False):
    """
    ...
    for k in gene_set_sizes:
        for model_name, model in [
            ...
            best_auc, best_genes = train_and_eval_loocv(model, X, y, k, feature_list, exhaustive)
    ...
    for model_type, ax in zip(["LogR-L1", "LogR-L2", "SVM"], axes):
        ...
        for dataset_name, (X_ds, y_ds, y_clean_ds, meta_ds) in plot_data_set_dict_use.items():
            ...
            if dataset_name in ["Hold_Out", "All"]:
            ...
            # Plot
            if dataset_name in ["Nano", "RNAseq", "All", "Hold_Out"]:
    ...
    #plt.savefig(f"../plots/SVM_RidgeR/{data_use}_NanoRNA_SVMRidge_AUCs.png", transparent=True, bbox_inches='tight')
    plt.show()
...
def get_coef_dfs(te_results_):
	lasso_coef_df = pd.DataFrame({
		...
		'Coefficient': te_results_["trained_models"]["LogR-L1"].coef_[0]
	})
	...
	ridge_coef_df = pd.DataFrame({
		...
		'Coefficient': te_results_["trained_models"]["LogR-L2"].coef_[0]
	})
	...
	svm_coef_df = pd.DataFrame({
		...
		'Coefficient': te_results_["trained_models"]["SVM"].coef_[0]
	})
...
def graph_coefs_PFS(coef_df, all_full_df, all_txn_df, corr_summary_df, title_use, scale=True, fig_width=15, RNAseqonly=False):
	if RNAseqonly:
...
def graph_sens_spec(metrics_df):
